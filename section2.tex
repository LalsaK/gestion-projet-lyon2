...
Notre groupe est constitué de deux étudiants. Afin de garantir une répartition équilibrée du travail, nous avons alterné les développements et les validations. L'historique des commits sur la branche principale montre clairement l'intégration régulière des contributions de chacun.
...
Nous avons créé un dépôt en ligne nommé \textbf{gestion-projet-lyon2} sur la plateforme GitHub. Ce dépôt étant privé pour la durée du développement, nous avons explicitement ajouté notre enseignant de CM en tant que collaborateur afin qu'il puisse y accéder et vérifier notre travail.
...
Pour éviter les conflits lors de la rédaction de ce compte rendu, nous avons utilisé des branches séparées pour nos axes de développement (ex: \texttt{feature-intro} et \texttt{feature-section2}). Chaque fusion vers la branche principale a fait l'objet d'une \textit{Pull Request} (PR) sur GitHub. Cela nous a permis d'effectuer des relectures croisées et d'interagir via les commentaires avant de valider le travail de l'autre.
...
La coordination de notre projet a été structurée autour de l'outil \textit{GitHub Projects}. Nous avons créé un tableau Kanban permettant de suivre l'avancement. L'ensemble de nos tâches a été défini via des \textit{Issues}, assignées à l'un ou l'autre des membres, et associées à un \textit{Milestone} spécifique pour notre rendu final.
...
Nous avons mis en place un processus d'intégration continue via \textit{GitHub Actions}. À chaque nouveau \textit{push} sur la branche principale, un script se déclenche automatiquement pour compiler notre code source \LaTeX{} et générer le document PDF final.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{images/6-Integration_continue.png}
    \caption{Exécution réussie de l'intégration continue pour la compilation \LaTeX.}
\end{figure}

\subsection*{[C3] Gestion de submodule}
Pour démontrer notre capacité à gérer une dépendance externe, nous avons intégré un \textit{submodule} Git à notre projet. Il s'agit d'un répertoire de templates \LaTeX{} \textit{open source}. L'enseignant peut récupérer l'intégralité du projet, incluant cette dépendance, en utilisant la commande \texttt{git clone --recursive}.
...
L'URL de notre site est la suivante : \url{https://lalsak.github.io/gestion-projet-lyon2/}
De plus, la version finale de ce document a été figée dans l'historique à l'aide d'un tag Git (version v1.1, faisant suite à une première version v1.0 avant assemblage).
